...
function transferListing(listing) {
  ...
  if(listing.ownerType.toLowerCase()=='supplier'){
    ...
    return getParticipantRegistry('composer.food.supply.Importer')
           .then(function(importerRegistry) {
                return importerRegistry.exists(listing.newOwner.getIdentifier());
           })
           .then(function(check) {
                if(check){
                    return  getAssetRegistry('composer.food.supply.ProductListingContract')
           ...
           .then(function(listingRegistry) {
                listingRegistry.update(productListing);
           });
      ...
      //  listing.ownerType='Retailer'
        ...
        return getAssetRegistry('composer.food.supply.ProductListingContract')
               ...
               .then(function(){
                    return getParticipantRegistry('composer.food.supply.Retailer');
               })
               .then(function(registry){
                    //listing.newOwner.products.concat(productListing.products);
                    productListing.products.forEach(function (item) {
                      listing.newOwner.products.push(item);
                    });
                    return registry.update(listing.newOwner);
               });
    }else{
